...
class TrainDataset(Dataset):
    def __init__(self, triples, nentity, nrelation, negative_sample_size, args=None):
        ...
        self.query_aware_dict1, self.query_aware_dict2 = self.read_qad(self.args.data_path + '/qt_dict.txt')
    ...
    def __getitem__(self, idx):
        ...
        #     pre_sample_list = self.pre_sampling1(head, relation, tail, pre_sample_num=self.args.pre_sample_size)
        # else:
        #     pre_sample_list = self.pre_sampling2(head, relation, tail, pre_sample_num=self.args.pre_sample_size)
        pre_sample_list = self.pre_sampling3(head, relation, tail, pre_sample_num=self.args.pre_sample_size)
        ...
        while negative_sample_size < self.negative_sample_size:
            ...
            mask = np.in1d(
                ...
                np.intersect1d(self.query_aware_dict2[(head, relation)], self.true_triples[(head, relation)], assume_unique=True),
                assume_unique=True, 
    ...
    def pre_sampling4(self, head, relation, tail, pre_sample_num=50, first_teacher_sampling=10):
        query_aware_tail1 = self.query_aware_dict1[(head, relation)][:first_teacher_sampling]
        query_aware_tail2 = self.query_aware_dict2[(head, relation)]
...
class TrainDataset_Random(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        positive_sample = torch.LongTensor(positive_sample)
        # positive_sample返回的是正确的三元组，negative_sample返回的是替换的entity号，subsampling_weight返回的是对应正确三元组的权重
        return positive_sample, negative_sample, subsampling_weight, 'RandomSample'
...
class TrueTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        true_triples = self.true_triples[(head, relation)]
        filter_true_triples = true_triples[true_triples!=tail]
        positive_sample_list.append(filter_true_triples)
