...
class GRU(nn.Module):
    def __init__(self, input_dim, position_dim, hidden_dim, output_dim, num_layers, seq_len, bidirectional=True, args=None):
        ...
        self.gru = nn.GRU(input_size=self.input_dim + self.position_dim, hidden_size=self.hidden_dim, batch_first=True,
    ...
    def forward(self, x):
        ...
        position_embeddings = torch.stack([param for param in self.position_embeddings])  # [seq_len, position_dim]
        position_embeddings = position_embeddings.unsqueeze(0).expand(batch_size, -1, -1)  # [batch, seq_len, position_dim]
...
class RNNExtracter(nn.Module):
    def __init__(self, args, mode=1):
        ...
        self.GRU = GRU(self.input_dim, self.position_dim, self.hidden_dim, self.output_dim, self.num_layers, self.seq_len,
                       bidirectional=self.bidirectional, args=None)
        self.fc = nn.Linear(self.hidden_dim * self.num_directions, self.hidden_dim)
        self.LDG = LowDimGenerate(hidden_dim=self.hidden_dim, input_dim=self.input_dim * self.seq_len,
                                  output_dim=self.output_dim, d_ff=args.t_dff)
