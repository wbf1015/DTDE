...
class ContrastiveLossv3(nn.Module):
    def __init__(self, args, stu_dim0, stu_dim1, stu_dim2, tea_dim1, tea_dim2, semantic_dim, layermul):
        """
        Contrastive Loss implementation.
        ...
        Args:
            tau (float): Temperature parameter for scaling.
        """
        super(ContrastiveLossv3, self).__init__()
        self.args = args
        self.tau  = self.args.contrastive_tau
        ...
        self.stu0_transform = Easy_MLP(args, input_dim=stu_dim0, output_dim=semantic_dim, layer_mul=layermul)
        # self.stu1_transform = Easy_MLP(args, input_dim=stu_dim1, output_dim=semantic_dim, layer_mul=layermul)
        # self.stu2_transform = Easy_MLP(args, input_dim=stu_dim2, output_dim=semantic_dim, layer_mul=layermul)
        self.tea1_transform = Easy_MLP(args, input_dim=tea_dim1, output_dim=semantic_dim, layer_mul=1)
        self.tea2_transform = Easy_MLP(args, input_dim=tea_dim2, output_dim=semantic_dim, layer_mul=1)
    
    def forward(self, stu0, stu1, stu2, tea1, tea2):
        stu0_semantic = self.stu0_transform(stu0)
        # stu1_semantic = self.stu1_transform(stu1)
        # stu2_semantic = self.stu2_transform(stu2)
        tea1_semantic = self.tea1_transform(tea1)
        tea2_semantic = self.tea2_transform(tea2)
        tea_semantic = 0.1*tea1_semantic + tea2_semantic
        
        if stu0.shape[1] > 1:
            loss0 = self.contrastive_similarityv2(stu0_semantic, tea_semantic)
            # loss1 = self.contrastive_similarityv2(stu1_semantic, tea_semantic)
            # loss2 = self.contrastive_similarityv2(stu2_semantic, tea_semantic)
        else:
            loss0 = self.contrastive_similarity(stu0_semantic, tea_semantic)
            # loss1 = self.contrastive_similarity(stu1_semantic, tea_semantic)
            # loss2 = self.contrastive_similarity(stu2_semantic, tea_semantic)
